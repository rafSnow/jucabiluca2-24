...
# Função para analisar a imagem e ajustar as configurações
def ajustar_configuracoes(img):
    img_np = np.array(img)
    ...
    # Detectar ruído (usando variância do Laplaciano)
    ruido = cv2.Laplacian(img_gray, cv2.CV_64F).var()
    
    # Verificar presença de bordas (usando Canny)
    bordas = cv2.Canny(img_gray, 100, 200).mean()
    
    # Ajustar configurações com base na análise
    configuracoes = {
        'equalizacao': cdf[int(0.5 * len(cdf))] < 0.5,  # Se a mediana do CDF for baixa, aplicar equalização
        'suavizacao': ruido > 100,      # Se a variância do Laplaciano for alta, aplicar suavização
        'nitidez': bordas < 50,         # Se a média das bordas for baixa, aplicar nitidez
        'brilho': brilho < 100,         # Se o brilho for baixo, aumentar brilho
        'contraste': contraste < 50     # Se o contraste for baixo, aumentar contraste
    }
    
    return configuracoes

# Função para atualizar o estado das checkboxes
def atualizar_checkboxes(configuracoes):
    var_equalizacao.set(configuracoes['equalizacao'])
    var_suavizacao.set(configuracoes['suavizacao'])
    var_nitidez.set(configuracoes['nitidez'])
    var_brilho.set(configuracoes['brilho'])
    var_contraste.set(configuracoes['contraste'])


# Função para aplicar pré-processamento na imagem
def pre_processar_imagem(img):
    configuracoes = ajustar_configuracoes(img)
    
    img_np = np.array(img)
    img_bgr = cv2.cvtColor(img_np, cv2.COLOR_RGB2BGR)

    if configuracoes['equalizacao']:
    ...
    if configuracoes['nitidez']:
        # Nitidez com kernel de nitidez mais agressivo
        kernel = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])
        img_sharp = cv2.filter2D(img_bgr, -1, kernel)
        img_bgr = cv2.addWeighted(img_bgr, 1.0, img_sharp, 0.5, 0)  # Combinar imagem original com a imagem nítida
        img_bgr = cv2.normalize(img_bgr, None, 0, 255, cv2.NORM_MINMAX)  # Normalizar para mitigar aumento de brilho

    if configuracoes['brilho'] or configuracoes['contraste']:
        # Ajuste de brilho e contraste com valores fixos
        alpha = 2 if configuracoes['contraste'] else 1.0  # Contraste (1.0 - 3.0)
        beta = 20 if configuracoes['brilho'] else 0       # Brilho (0 - 100)
        
        # Ajuste de brilho e contraste sem usar convertScaleAbs
        img_bgr = img_bgr.astype(np.float32)  # Converter para float para permitir valores negativos
        img_bgr = alpha * img_bgr + beta
        img_bgr = np.clip(img_bgr, 0, 255)  # Garantir que os valores estejam no intervalo [0, 255]
        img_bgr = img_bgr.astype(np.uint8)  # Converter de volta para uint8
...
chk_contraste = tk.Checkbutton(frame_preprocessamento, text="Contraste", variable=var_contraste, bg="#f0f0f0", font=("Arial", 12))
chk_contraste.pack(side="left", padx=10, pady=10)
chk_contraste.bind("<Enter>", lambda event: mostrar_descricao(event, "Contraste: Ajusta o contraste da imagem para realçar as diferenças entre os níveis de intensidade. \nIsso pode melhorar a detecção de objetos ao tornar os detalhes mais nítidos, mas pode saturar a imagem se usado em excesso."))
chk_contraste.bind("<Leave>", esconder_descricao)
