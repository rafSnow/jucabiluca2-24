...
def pre_processar_imagem(img):
    ...
    if var_nitidez.get():
        ...
        nitidez_valor = slider_nitidez.get()  # Obter valor do slider
        kernel = np.array([[0, -1, 0], [-1, 4 + nitidez_valor, -1], [0, -1, 0]])  # Ajustar o kernel com base no valor do slider
    ...
    if var_brilho.get():
        # Ajuste de brilho e contraste com sliders
        alpha = slider_contraste.get()  # Contraste
        beta = slider_brilho.get()      # Brilho
...
# Frame para os sliders de parâmetros
frame_sliders = tk.Frame(root, bg="#f0f0f0")
frame_sliders.pack(side="top", pady=10)

# Suavização: Slider para o tamanho do kernel
label_suavizacao_slider = tk.Label(frame_sliders, text="Tamanho do Kernel (Suavização)", bg="#f0f0f0", font=("Arial", 12))
label_suavizacao_slider.pack(side="left", padx=10)
slider_suavizacao = tk.Scale(frame_sliders, from_=1, to=15, orient="horizontal", bg="#f0f0f0", font=("Arial", 12), length=200)
slider_suavizacao.set(5)  # Valor padrão (kernel 5x5)
slider_suavizacao.pack(side="left", padx=10)

# Brilho: Slider para ajuste de brilho
label_brilho_slider = tk.Label(frame_sliders, text="Ajuste de Brilho", bg="#f0f0f0", font=("Arial", 12))
label_brilho_slider.pack(side="left", padx=10)
slider_brilho = tk.Scale(frame_sliders, from_=0, to=100, orient="horizontal", bg="#f0f0f0", font=("Arial", 12), length=200)
slider_brilho.set(50)  # Valor padrão
slider_brilho.pack(side="left", padx=10)

# Contraste: Slider para ajuste de contraste
label_contraste_slider = tk.Label(frame_sliders, text="Ajuste de Contraste", bg="#f0f0f0", font=("Arial", 12))
label_contraste_slider.pack(side="left", padx=10)
slider_contraste = tk.Scale(frame_sliders, from_=0.5, to=3.0, resolution=0.1, orient="horizontal", bg="#f0f0f0", font=("Arial", 12), length=200)
slider_contraste.set(1.2)  # Valor padrão
slider_contraste.pack(side="left", padx=10)

# Nitidez: Slider para ajuste de nitidez
label_nitidez_slider = tk.Label(frame_sliders, text="Ajuste de Nitidez", bg="#f0f0f0", font=("Arial", 12))
label_nitidez_slider.pack(side="left", padx=10)
slider_nitidez = tk.Scale(frame_sliders, from_=1, to=10, orient="horizontal", bg="#f0f0f0", font=("Arial", 12), length=200)
slider_nitidez.set(5)  # Valor padrão
slider_nitidez.pack(side="left", padx=10)
...
root.update_idletasks()
root.geometry('1600x1000')  # Defina um tamanho inicial para a janela
centralizar_janela()
